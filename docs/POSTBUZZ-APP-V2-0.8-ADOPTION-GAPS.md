...
- **`geminiAdapter`/`geminiPricingSource` still exist as raw exports** in
  0.8.0 (confirmed in `packages/google/src/index.ts`). The 0.8.0 contract for
  client assembly is `composeProviders([googleProvider()])`, which supplies
  the now-required `modelRegistry` (mandatory with no default): the host must
  replace its current hand-assembled
  `createClient({ adapters: [geminiAdapter()], pricingSources: {...} })` with
  that composition as a required migration step when it adopts 0.8.0.
...
`composeProviders` already supports. The package is new surface with its own
contract; the host adopts it by replacing its direct `@anthropic-ai/sdk`
usage with the plugin, and is a ready real-world consumer the moment it
exists.
